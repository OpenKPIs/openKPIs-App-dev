...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!apiKey) return NextResponse.json({ error: 'No API key. Open ⚙ Settings to add one.' }, { status: 401 });
    ...
    const prompt = `You are an expert analytics consultant specializing in digital measurement and KPI frameworks.

Task: Generate complete, production-ready ${entityType.toUpperCase()} definitions for every item listed below.
${ctx ? `\nContext:\n${ctx}\n` : ''}
Items:
${itemList}

Fill EVERY field for EACH item. Be specific and actionable.

Fields:
${fieldList}

Return ONLY valid JSON — no markdown, no explanation:
{"items": [{"name": "exact item name", ...all fields...}, ...]}`;

    let raw: string;
    if (provider === 'anthropic') raw = await callAnthropic(prompt, model, apiKey);
    else if (provider === 'google') raw = await callGoogle(prompt, model, apiKey);
    else if (provider === 'custom') raw = await callCustom(prompt, model, apiKey, baseUrl ?? 'http://localhost:11434/v1');
    else raw = await callOpenAI(prompt, model, apiKey);
    ...
    if (Array.isArray(parsed)) results = parsed;
      ...
      if (Array.isArray(obj.items)) results = obj.items;
        ...
        results = k ? obj[k] as unknown[] : [obj];
      }
    }
